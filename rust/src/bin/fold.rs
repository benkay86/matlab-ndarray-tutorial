...
fn main() {
    ...
    println!(
        "Sum of rows using fold_axis() =\n{:?}",
        mat.fold_axis(Axis(1), 0., |sum, col| sum + col)
    );
    println!(
        "Sum of rows using sum_axis() =\n{:?}\n",
        mat.sum_axis(Axis(1))
    );
    ...
    let sum = Zip::from(&mat)
        .and(&mat)
        .fold(0., |sum, el1, el2| sum + el1 + el2);
    ...
    let ans = mat
        .iter()
        .try_fold(false, |ans, el| {
            if el == &2. {
                Err(())
            }
            // causes try_fold to return early
            else {
                Ok(ans)
            }
        })
